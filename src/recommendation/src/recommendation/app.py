...
from resolvers import DefaultProductResolver, PersonalizeRecommendationsResolver, PersonalizeRankingResolver, RankingProductsNoOpResolver, PersonalizeContextComparePickResolver, RandomPickResolver, IncrementalResolver
from utils import CompatEncoder
...
def get_recipe(arn):
    ...
    if resource:
        if is_recommender:
            ...
            solution_version = personalize_meta_cache.get(
                resource['solutionVersionArn'])

            if not solution_version:
                response = personalize.describe_solution_version(
                    solutionVersionArn=resource['solutionVersionArn'])
                if response.get('solutionVersion'):
                    solution_version = response['solutionVersion']
                    personalize_meta_cache[resource['solutionVersionArn']
                                           ] = solution_version
...
def get_parameter_values(names):
    ...
    parameter_store = {
        '/retaildemostore/personalize/popular-items-arn': os.getenv('POPULAR_ITEMS_ARN'),
        '/retaildemostore/personalize/personalized-ranking-arn': os.getenv('PERSONALIZED_RANKING_ARN'),
        '/retaildemostore/personalize/related-items-arn': os.getenv('RELATED_ITEMS_ARN'),
        '/retaildemostore/personalize/recommended-for-you-arn': os.getenv('RECOMMENDED_FOR_YOU_ARN'),
    }

    values = []

    if names[0] in parameter_store:
        values.append(parameter_store[names[0]])
        values.append("")
        return values
    ...
    assert len(values) == len(
        names), 'mismatch in number of values returned for names'
...
def fetch_product_details(item_ids: Union[str, List[str]], fully_qualify_image_urls=False) -> List[Dict]:
    ...
    item_ids_csv = item_ids if isinstance(
        item_ids, str) else ','.join(item_ids)
...
def get_products(feature, user_id, current_item_id, num_results, default_inference_arn_param_name,
    ...
    # Fallback to default behavior of checking for campaign/recommender ARN parameter anexd
    # then the default product resolver.
    values = get_parameter_values(
        [default_inference_arn_param_name, default_filter_arn_param_name])
    ...
    if inference_arn and (user_id or not user_reqd_for_inference):
        ...
        logger.info(
            f"get_products: Supplied campaign/recommender: {inference_arn} (from {default_inference_arn_param_name}) Supplied filter: {filter_arn} (from {default_filter_arn_param_name}) Supplied user: {user_id}")

        resolver = PersonalizeRecommendationsResolver(
            inference_arn=inference_arn, filter_arn=filter_arn)
        ...
        resolver = DefaultProductResolver(
            products_service_host=products_service_host, products_service_port=products_service_port)

        items = resolver.get_items(
            product_id=current_item_id, num_results=num_results)
...
app = Flask(__name__)
logger = app.logger
corps = CORS(app, expose_headers=[
             'X-Experiment-Name', 'X-Experiment-Type', 'X-Experiment-Id', 'X-Personalize-Recipe'])
...
def related():
    ...
    filter_ssm = request.args.get(
        'filter', filter_include_categories_param_name)
    ...
    fully_qualify_image_urls = request.args.get(
        'fullyQualifyImageUrls', '0').lower() in ['true', 't', '1']
...
def create_user():
    """After you create a Users dataset, you can incrementally import one or more new users into the dataset. 
    Incrementally importing users allows you to keep your Users dataset current as your business grows. 
    If you have a large amount of new users, we recommend that you first import data in bulk and then import user data incrementally as necessary. See Importing bulk records.
    You can use the Amazon Personalize console, the AWS Command Line Interface (AWS CLI), or AWS SDKs to import users. 
    If you import a user with the same userId as a user that's already in your Users dataset, 
    Amazon Personalize replaces the user with the new one. You can import up to 10 users at a time.
    For information about how Amazon Personalize updates filters for new records and how new records 
    influence recommendations, see Importing records incrementally.
    """
    resolver = IncrementalResolver()
    user = request.get_json()

    try:
        response = resolver.create_user(user)
        app.logger.debug(
            f"Recommendations response to be returned: {response}")
        return response

    except Exception as e:
        app.logger.exception('Unexpected error creating user', e)
        raise BadRequest(message='Unhandled error', status_code=500)
...
def get_ranking(user_id, items, feature,
    ...
    # then the default product resolver.
    values = get_parameter_values(
        [default_inference_arn_param_name, filter_purchased_param_name])
...
def choose_discounted():
    ...
    try:
        user_id, items, feature = ranking_request_params()
        response_items, resp_headers = get_top_n(
            user_id, items, feature, NUM_DISCOUNTS)
        ...
        app.logger.exception(
            'Unexpected error calculating discounted items', e)
        return json.dumps(items)
...
def get_all_offers_by_id():
    ...
    if not offers_response.ok:
        logger.error(
            f"Offers service not giving us offers: {offers_response.reason}")
...
def coupon_offer():
    ...
    try:
        ...
        if not offers_response.ok:
            app.logger.exception(
                'Offers service did not return happily', offers_response.reason())
            raise BadRequest(message='Cannot obtain offers', status_code=500)
        else:
            ...
            if not inference_arn:
                app.logger.warning(
                    'No campaign Arn set for offers - returning arbitrary')
                ...
                logger.info(
                    f'Recommendations returned: {json.dumps(get_recommendations_response)}')
                ...
                logger.info(
                    f"Scores after adjusting for preference parameters: {adjusted_scores}")
                ...
                logger.info(
                    f"Scores after normalising adjusted scores: {adjusted_scores}")
                ...
                adjusted_scores = {offer_id: score*(1-random_factor) + random_factor*random.random()
                                   for offer_id, score in adjusted_scores.items()}
                logger.info(
                    f"Scores after adding randomness: {adjusted_scores}")
        ...
        app.logger.debug(
            f"Recommendations response to be returned for offers: {resp}")
        return resp
