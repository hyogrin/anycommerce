...
class DefaultProductResolver(Resolver):
    ...
    def __init__(self, **params):
        ...
        if not self.products_service_host:
            ...
            log.debug('DefaultProductResolver - fetched product service instance %s',
                      self.products_service_host)
        else:
            log.debug('DefaultProductResolver - using product service instance %s',
                      self.products_service_host)

        self.fully_qualify_image_urls = params.get(
    ...
    def get_items(self, **kwargs):
        ...
        if product_id:
            ...
            try:
                ...
                log.error(
                    "Could not pull product information from URL %s - error: %s", url, e)
        ...
        if category:
            ...
            log.debug(
                'DefaultProductResolver - getting products for category %s', url)
            response = requests.get(url)
        else:
            ...
            log.debug(
                'DefaultProductResolver - getting featured products %s', url)
            response = requests.get(url)
        ...
        if response.ok:
            ...
            raise Exception(
                f'Error calling products service: {response.status_code}: {response.reason}')
...
class IncrementalResolver():
    def create_user(self, user):
        ...
        user['selectable_user'] = str(user['selectable_user'])

        new_user = {}
        new_user['properties'] = {}
        for k in user:
            new_user['properties'][k] = user[k]

        new_user['userId'] = temp
        new_user['properties'] = json.dumps(new_user['properties'])

        response = personalize_events.put_users(
            datasetArn=os.getenv('DATASET_ARN'),
            users=[new_user],
        )
...
class SearchSimilarProductsResolver(Resolver):
    ...
    def __init__(self, **params):
        ...
        if not self.search_service_host:
            ...
            log.debug('SearchSimilarProductsResolver - fetched search service instance %s',
                      self.search_service_host)
        else:
            log.debug('SearchSimilarProductsResolver - using search service instance %s',
                      self.search_service_host)
    ...
    def get_items(self, **kwargs):
        ...
        log.debug(
            'SearchSimilarProductsResolver - getting similar products %s', url)
        response = requests.get(url)
...
class PersonalizeRecommendationsResolver(Resolver):
    ...
    def __init__(self, **params):
        ...
        if not self.inference_arn:
            raise Exception(
                'inference_arn required for PersonalizeRecommendationsResolver')
    ...
    def get_items(self, **kwargs):
        ...
        is_recommender = self.inference_arn.split(
            ':')[5].startswith('recommender/')
        if is_recommender:
        ...
        log.debug(
            'PersonalizeRecommendationsResolver - getting recommendations %s', params)

        response = PersonalizeRecommendationsResolver.__personalize_runtime.get_recommendations(
            **params)
...
class HttpResolver(Resolver):
    ...
    def __init__(self, **params):
        ...
        self.user_id_parameter_name = params.get(
            'user_id_parameter_name', 'userId')
        self.item_id_parameter_name = params.get(
            'item_id_parameter_name', 'itemId')
        self.num_results_parameter_name = params.get(
            'num_results_parameter_name', 'numResults')
    ...
    def get_items(self, **kwargs):
        ...
        if response.ok:
            ...
            raise Exception(
                f'Error calling HTTP endpoint service: {response.status_code}: {response.reason}')
...
class PersonalizeRankingResolver(Resolver):
    ...
    def __init__(self, **params):
        ...
        if not self.inference_arn:
            raise Exception(
                'inference_arn required for PersonalizeRankingResolver')
    ...
    def get_items(self, **kwargs):
        ...
        log.debug(
            'PersonalizeRankingResolver - getting personalized ranking %s', params)

        response = PersonalizeRankingResolver.__personalize_runtime.get_personalized_ranking(
            **params)
...
class PersonalizeContextComparePickResolver(Resolver):
    ...
    def __init__(self, **params):
        ...
        self.with_resolver = PersonalizeRankingResolver(
            **params, context=with_context)
        self.without_resolver = PersonalizeRankingResolver(
            **params, context=without_context)
    ...
    def get_items(self, **kwargs):
        ...
        log.debug(
            'PersonalizeContextComparePickResolver - comparing personalized rankings...')
        ...
        discount_improve_sorted_items = [
            with_id_to_item[item_id] for item_id in discount_improve_sorted_item_ids]
...
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_PRODUCT, DefaultProductResolver)
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_SIMILAR, SearchSimilarProductsResolver)
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_PERSONALIZE_RECOMMENDATIONS, PersonalizeRecommendationsResolver)
...
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_PERSONALIZE_RANKING, PersonalizeRankingResolver)
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_RANKING_NO_OP, RankingProductsNoOpResolver)
# These ones are for the top-N use cases
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_PERSONALIZE_PICK, PersonalizeContextComparePickResolver)
ResolverFactory.register_resolver(
    ResolverFactory.TYPE_RANDOM_PICK, RandomPickResolver)
