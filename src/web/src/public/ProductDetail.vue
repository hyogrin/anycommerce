      ...
      <div class="container">
        <main class="product-container mb-5 text-left">
          ...
          <div class="add-to-cart-and-description">
            ...
            <div class="mb-2">
              <template v-if="outOfStock">현재 상품이 재고가 남아있지 않습니다.</template>
              <template v-else>남아있는 수량: {{ product.stock }}</template>
            </div>

            <!-- <div v-if="fenixenablePDP == 'TRUE'" class="fenix-estimates">
              <Fenixmaster :currentvariant="fenixcurrentvariant">
              </Fenixmaster>
            </div> -->

            <div v-if="cartHasMaxAmount" class="mb-2">이 상품을 장바구니에 더 추가할 수 없습니다.</div>

            <div class="mb-5 mb-md-4 d-flex">
              <button v-if="!outOfStock && !cartHasMaxAmount"
                class="quantity-dropdown mr-3 btn btn-outline-secondary dropdown-toggle" type="button"
                id="quantity-dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                수량: {{ quantity }}
              </button>
              <div class="dropdown-menu" aria-labelledby="quantity-dropdown">
                <button v-for="i in Math.max(0, Math.min(9, product.stock - quantityInCart))" :key="i"
                  class="dropdown-item" @click="quantity = i">
                  {{ i }}
                </button>
              </div>
  ...
  computed: {
    ...
    previousPageLinkProps() {
      ...
      return {
        ...
        text: I18n['categories'][this.readableProductCategory.toLowerCase()],
      };
    },
    cartItem() {
    ...
    cartHasMaxAmount() {
      ...
      return !this.outOfStock && this.cartItem.quantity >= this.product.stock;
    },
  },
  watch: {
    $route: {
