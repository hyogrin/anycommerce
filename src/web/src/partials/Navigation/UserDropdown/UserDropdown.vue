<template>
  <div>
    <router-link v-if="!user" to="/auth" class="user-dropdown-button login-button btn">로그인</router-link>
    <router-link v-if="!user" to="/register" class="user-dropdown-button login-button btn">회원가입</router-link>


    <button v-if="user" id="navbarDropdown"
    ...
    <div v-if="user" class="dropdown-menu px-3" aria-labelledby="navbarDropdown">
      <a href="#" @click="switchShopper" class="dropdown-item">
        ...
        <div>다른 사용자로 전환할 수 있습니다!</div>
      </a>

      <div class="dropdown-divider"></div>

      <router-link to="/orders" class="dropdown-item">
        <div class="dropdown-item-title">주문내역</div>
        <div>주문 내역 확인하기</div>
      </router-link>

      <div class="dropdown-divider"></div>

      <button class="dropdown-item dropdown-item-title" @click="logout">로그아웃</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    logout() {
      EventBus.$emit('authState', 'signedOut');
      swal('로그아웃 되었습니다.');
    },
  },
};
