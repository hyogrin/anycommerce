<template>
  <div class="d-inline-flex dropdown">
    ...
    <input type="text" autocomplete="off" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="search"
      class="input py-2" v-model="searchTerm" @focus="onInputFocus" @blur="onInputBlur" />
    <ul v-show="searchTerm !== ''" class="search-results dropdown-menu" role="menu" aria-labelledby="search">
      ...
      <SearchItem v-for="result in results" :key="result.itemId" :product="result.product"
        :experiment="result.experiment" :feature="feature" />
    </ul>
  </div>
</template>
        ...
        .catch(function (error) {
          if (error.response) {
            if (error.response.status == 404) {
              comp.searchError = '아직 구현되지 않았습니다.'
            }
            else {
              comp.searchError = `아직 구현되지 않았습니다.`;
            }
